...
//      实现pagemodel组件，新增按钮的功能，使用elementplus的dialog组件，利用之前封装的z2qFrom组件来快速搭建，更改样式就可以。预留插槽做定制化
//      实现按钮功能，content组件中的按钮使用emit传递事件给user，uesr去做操作。具体的操作放到hooks中
//      做完响应式数据的处理如果有问题，可能不是代码的问题，热更新问题，重启一下服务
//        tips: 目前form内的双向绑定的方式不能触发响应式，在修改单个数据之后才能够触发后续响应式。在pagemodel中这个问题能够借助eldialog的destroy-on-close属性解决
//      通过修改配置文件，增加是否显示的选项，来决定搜索表单的选项是否要隐藏，并在使用的界面里写回调函数，user页面
//      请求部门和角色的数据，保存在vuex的root中。注意vuex的网络请求都是异步的，有可能数据还没请求下来，组件就搭好了，同时登录时token也是异步的，有可能token还没下来数据就请求到了
//      要把需要赋值的数据用computed包裹，并把赋值的语句放到computed副作用函数里面去，这样就会在数据到来的时候自动更新
//    14.实现新建和编辑，先监听事件点击，然后dispatch异步操作getPageListAction发送网络请求。发送网络请求后刷新
//    15.实现退出登录逻辑，删除token等数据，并告诉后端用户退出登录
//    16.搭建role的新建角色逻辑，通过配置文件快速搭建，抽离单独属于role的逻辑放到role界面,通过插槽插入model组件
//       利用eltree组件实现权限选择，拿到完整的菜单，存在vuex中。传递选中数据给model。
//       实现回显，这里只选中叶子节点，因为选中叶子节点就会自动选上上一级的父节点，直接选父节点会全选。
//       利用callback函数拿到数据，创建一个map映射，通过菜单拿到tree的数组，在绑定到组件上。注意 组件在创建时，还来不及和数据绑定，可以用next来推迟到下一帧操作
//    17.使用echarts，工程化平台直接npm install echarts就可以。使用步骤1.初始化实例，2.创建echarts配置文件，3.开始绘制。查看官网
//    18.搭建dashboard组件，创建对应的vuex模块，请求数据
//       封装z2qcard组件,用来展示图片。搭建dashboard布局。使用<script>setup特性来编写一个新组件echart（3.2.0以上才能使用）
//       把echart的逻辑全部抽到一个hook里面，方便以后更换。同时做浏览器大小变化适配。
//       因为图形画好之后基本不变，变的都是数据，可以再做一层封装，对每种图形都做一个组件，常用的options写死，暴漏出需要更改的options如data。
//       找不到想要的图表可以去echarts的第三方社区找。地图类的数据，先画地图，一般去网上请求地图数据然后注册，然后画自己的数据
//         tips:绘制地图：1.注册地图。拿到地图数据china.json(不认识json文件去.d.ts声明)，去hooks里面注册地图。
//                       2.创建mapEcharts组件，设置配置options。对value数据做转化，转成经纬度+数据的形式，写一个hooks
//                       3.拿到经纬度的数据，用来给hooks进行映射最终使用。
//    19.项目打包自动化部署。
export default {}
</script>
